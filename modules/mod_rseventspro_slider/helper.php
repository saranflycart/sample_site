...
abstract class modRseventsProSlider {
	...
	public static function getEvents($params) {
		...
		if ($return = $db->loadObjectList()) {
			foreach ($return as &$event) {
				if (!empty($event->icon)) {
					if ($image_type == 0) {
						$event->image = JRoute::_('index.php?option=com_rseventspro&task=image&id='.rseventsproHelper::sef($event->id,$event->name), false);
					} elseif ($image_type == 1) {
						$event->image = JRoute::_('index.php?option=com_rseventspro&task=image&id='.rseventsproHelper::sef($event->id,$event->name).'&width=big', false);
					} elseif ($image_type == 2) {
						$event->image = JURI::root().'components/com_rseventspro/assets/images/events/'.$event->icon;
					} else {
						$event->image = JRoute::_('index.php?option=com_rseventspro&task=image&id='.rseventsproHelper::sef($event->id,$event->name).'&width='.$custom, false);
					}
				}
			}
		}
	...
	public static function carousel($selector, $params) {
		...
		if (!$is30) {
			if ($params->get('responsive_jquery',1)) {
				$jqueryHelper = RSEventsSliderjQuery::getInstance();
				$jqueryHelper->addjQuery();
			}
		}
		
		$document->addStyleSheet($root.'/modules/mod_rseventspro_slider/assets/css/responsive/bootstrap-carousel.css');
		$document->addStyleSheet($root.'/modules/mod_rseventspro_slider/assets/css/responsive/j'.($is30 ? '30' : '25').'-fixes.css');
		$document->addStyleSheet($root.'/modules/mod_rseventspro_slider/assets/css/responsive/'.$theme.'.css');
		$document->addScript($root.'/modules/mod_rseventspro_slider/assets/js/jcarousel.js?v=2');
		$document->addScript($root.'/modules/mod_rseventspro_slider/assets/js/jquery.touchSwipe.min.js?v=2');
...
function _iniRSEventsSliderjQuery() {
	$jqueryHelper 	= RSEventsSliderjQuery::getInstance();
...
class RSEventsSliderjQuery
{
	...
	public static function getInstance() {
		return new RSEventsSliderjQuery();
	}
	...
	public function addjQuery() {
		if ($this->mode == 'auto') {
			...
			$mainframe->registerEvent('onAfterRender', '_iniRSEventsSliderjQuery');
		}
	...
	protected function addScript() {
		...
		$document->addScript(JURI::root(true).'/modules/mod_rseventspro_slider/assets/js/jquery'.$src.'.js');
		$document->addScript(JURI::root(true).'/modules/mod_rseventspro_slider/assets/js/jquery-noconflict.js');
	}

	public function foundMultiplejQuery($where=null) {
		$body = $where ? $where : JResponse::getBody();
		// find jQuery versions
		return preg_match_all($this->jQueryPattern, $body, $matches);
	}
	...
	public function replacejQuery($where=null) {
		...
		// jQuery
		$jquery 	= '<script type="text/javascript" src="'.JURI::root(true).'/modules/mod_rseventspro_slider/assets/js/jquery'.$src.'.js'.'"></script>';
		// jQuery noConflict() mode enforcer
		$noconflict = '<script type="text/javascript" src="'.JURI::root(true).'/modules/mod_rseventspro_slider/assets/js/jquery-noconflict.js'.'"></script>';
