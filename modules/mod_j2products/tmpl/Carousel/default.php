	...
	(function($) {
		$(document).ready(function() {
			$("#<?php echo $slider_id;?>").owlCarousel({
				singleItem:<?php echo $forced_singleItem;?>,
				items:<?php echo $total_cols;?>,
                itemsDesktop : [1200,<?php echo $total_colsDesktop;?>],
                itemsDesktopSmall : [992,<?php echo $total_colsDesktopSmall;?>],
                itemsTablet : [768,<?php echo $total_colsTablet;?>],
                itemsTabletSmall : [480,<?php echo $total_colsTabletSmall;?>],
                itemsMobile : [320,<?php echo $total_colsMobile;?>],
				autoPlay:<?php echo $slider_autoplay;?>,
				pagination:<?php echo $show_pagination;?>,
				navigation:<?php echo $show_navigation;?>,
				stopOnHover:true,
				navigationText: [
					"<i class='fa fa-caret-left icon-chevron-left icon-white'></i>",
...
<?php foreach ($list as $product_id => $product) : ?>
	...
	<div class="product-cart-left-block <?php echo $img_class; ?>" >
		...
		<div class="product_cart_block">
			...
			<?php if($j2params->get('catalog_mode', 0) == 0 && $product->show_cart): ?>
				...
				<?php
					...
					if ($product->product_type=='configurable') {
						$product->show_options = 2	;
					}

					if($product->show_options == 1) : ?>
					...
					?>
				<?php elseif( $product->product_type=='configurable' || ($product->show_options == 2 && count($product->options)) ):?>
